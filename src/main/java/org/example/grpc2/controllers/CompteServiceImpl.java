package org.example.grpc2.controllers;

import io.grpc.stub.StreamObserver;
import net.devh.boot.grpc.server.service.GrpcService;
import org.example.grpc2.services.CompteService;
import org.example.grpc2.stubs.*;
...
public class CompteServiceImpl extends CompteServiceGrpc.CompteServiceImplBase {
    ...
    public void totalSolde(GetTotalSoldeRequest request, StreamObserver<GetTotalSoldeResponse> responseObserver) {
        Map<String, Number> statsMap = compteService.getTotalSoldeStats();

        SoldeStats stats = SoldeStats.newBuilder()
                .setCount(statsMap.get("count").intValue())
                .setSum(statsMap.get("sum").floatValue())
                .setAverage(statsMap.get("average").floatValue())
                .build();
    ...
    public void saveCompte(SaveCompteRequest request, StreamObserver<SaveCompteResponse> responseObserver) {
        ...
        // CORRECTION ICI : Utilisation du bon package pour l'Entité JPA
        var compteEntity = new org.example.grpc2.entities.Compte();
        ...
        responseObserver.onNext(SaveCompteResponse.newBuilder()
                .setCompte((org.example.grpc2.stubs.Compte) grpcCompte).build());
        responseObserver.onCompleted();
    }
}